  ...
  <section class="p-sec">
    <div class="container">
      ...
      <form class="form-items" ref="profile"  @submit.prevent="updateProfile">
        ...
        <div class="user-img">
              <img :src="user_image" />
              <input type="file" name="photo" class="photo_image" value="" @change="updateImage">
        </div>
        ...
        <div class="grid-two">
          ...
          <div class="form-item">
            ...
            <select v-model="city_id" class="form-item-input" id="inputGroupSelect01">
          ...
          <div class="form-item">
           ...
           <textarea v-model="desc_ar" name="desc_ar" class="form-item-input" id="" cols="30" rows="2"></textarea>
          </div>
        </div>
        <!-- End Grid Two -->

        <div class="form-group">
          <button class="global-button" :disabled="disabled"> حفظ التعديلات </button>
        </div>
      </form>
    </div>
  </section>
...
export default {
    ...
    methods: {
        ...
        async getCities() {
            await axios.post('https://dashboard.mueadaat.info/test-mode/api/sections', {
                lang: localStorage.getItem('locale'),
                user_id : JSON.parse(sessionStorage.getItem( "user" )).data.id
            })
            .then((res) => {
        ...
        async updateProfile() {
            ...
            await axios.post('https://dashboard.mueadaat.info/test-mode/api/update-user', fd)
                .then((res) => {
                    if (res.data.key == 1) {
